...
(function (ps_graphDefinitions, $, undefined) {
	...
	*/
    ...
    ps_graphDefinitions.buildBarChart = function (sElementName) {
        ...
        objChart.getAxisY().getDataFormat().setDecimals(2);
        ...
        '<DataTemplate>' +
            '<DockPanel Orientation="Horizontal" Margin="3,0,3,0">' +
                ...
                '<TextBlock Text="{Binding Path=Macro %P%%}" FontWeight="Bold" HorizontalAlignment="Right"/>' +
            '</DockPanel>' +
        '</DataTemplate>';
    ...
    ps_graphDefinitions.buildSentimentCompetitors = function (sElementName) {
        ...
        var data = ps_graphDefinitions.jsonData.data;
        var dataSort = _.sortBy(data, function(val){return val.display_2;});
	...
	*/
    ps_graphDefinitions.buildLineChart = function (sElementName) {
        ...
        objChart.getToolTips().setAllSeries(false);
        objChart.getAxisY().getLabelsFormat().setFormat(cfx.AxisFormat.Number);
        ...
        $(divHolder).mousemove(function(e) {
            $('#'+sElementName.div_location+' #chartToolTip'+sElementName.div_location).css('left', e.pageX - 40 - $(this).offset().left);
            $('#'+sElementName.div_location+' #chartToolTip'+sElementName.div_location).css('top', e.pageY - 110 - $(this).offset().top);
        });
	...
	*/
    ps_graphDefinitions.buildLinkedinFollowers = function (sElementName) {
        ...
        var axisX = objChart.getAxisX();
        var axisY = objChart.getAxisY();
        ...
        objChart.getLegendBox().sizeToFit();
        axisX.setMinorStep(1);
        axisX.getGrids().getMinor().setVisible(true);
        objChart.getAnimations().getLoad().setEnabled(true);

        var dataSort = sElementName.jsonData.data;
        	 
        var data = ps_utilities.processDataLinkedinFollowers(dataSort);
        var min = dataSort[0].num_followers - 600;
        var max = dataSort[dataSort.length-1].num_followers + 400;
        axisY.setMin(min);
        axisY.setMax(max);
        axisY.setMinorStep(0);
        axisY.setStep(200);
        axisY.getLabelsFormat().setFormat(cfx.AxisFormat.Number);
	...
	*/
    ps_graphDefinitions.buildKeywordTrending = function(sElementName) {
        ...
        $.each(ps_graphDefinitions.jsonData.data, function(i,v){
           $("#"+sElementName.category).append("<li class = 'keyword-term'  tag=' +v.display+'>"  + v.display+"<span class='keyword-value'>"+ v.value + "</span>"+ "</li>");
		});
